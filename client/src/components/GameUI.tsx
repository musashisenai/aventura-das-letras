...
import { ArrowLeft, BookOpen, Check, ChevronRight, CircleHelp, Coins, Eye, EyeOff, Gift, Heart, Lock, Paintbrush, PawPrint, Play, RotateCcw, Sparkles, Star, Volume2, VolumeX, X } from "lucide-react";
...
type RemoteStudent = { id: string; profile: { name: string; currentWorld: number; xp: number; petName: string }; completions: Record<string, unknown>; answers: unknown[]; updatedAt: string };
  ...
  return <main className="entry-menu-page">
    ...
    <section className="entry-menu-copy paper-panel">
      ...
      <div className="entry-actions">
        {state.profile ? <><button className="entry-path child-path" onClick={() => controller.continueSave()}><i className="entry-stop-tag" aria-hidden="true">CONTINUAR</i><span className="entry-icon"><Play size={25} fill="currentColor" /></span><span><strong>Continuar aventura</strong><small>Voltar para o mapa de {state.profile.name}</small></span><ChevronRight size={24} /></button><button className="entry-path" onClick={() => controller.startNewSave()}><i className="entry-stop-tag" aria-hidden="true">NOVA TRILHA</i><span className="entry-icon"><Sparkles size={25} /></span><span><strong>Começar novo save</strong><small>Montar uma nova aventura</small></span><ChevronRight size={24} /></button></> : <button className="entry-path child-path" onClick={() => controller.openPlayerSetup()}><i className="entry-stop-tag" aria-hidden="true">PARADA 01</i><span className="entry-icon"><PawPrint size={25} /></span><span><strong>Sou criança</strong><small>Abrir minha expedição</small></span><ChevronRight size={24} /></button>}
  ...
  return (
    <main className="welcome-page">
      ...
      <section className="welcome-copy paper-panel">
        ...
        <button className="primary-action" onClick={() => state.profile ? controller.updateProfileName(name) : controller.beginProfile(name)}>{state.profile ? "Salvar alterações" : "Começar a expedição"} <ChevronRight size={23} /></button>
...
function Teacher({ state, controller }: Props) {
  ...
  useEffect(() => {
    ...
    const refresh = () => fetch("/api/students").then((response) => response.ok ? response.json() : []).then((students: RemoteStudent[]) => setRemoteStudents(students)).catch(() => undefined);
    refresh();
    const timer = window.setInterval(refresh, 4000);
    return () => window.clearInterval(timer);
  }, [state.teacherAuthorized]);
  const profile = state.profile;
  if (!state.teacherAuthorized) return <main className="single-game-page"><section className="teacher-lock paper-panel"><Lock size={38} /><p className="eyebrow">Acesso orientador</p><h1>Área do professor</h1><p>Entre para acompanhar a trilha, as respostas e os desenhos de cada aluno neste dispositivo.</p><div className="password-input-wrap"><input type={showPassword ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Senha" autoComplete="current-password" /><button type="button" className="password-eye" onClick={() => setShowPassword((visible) => !visible)} aria-pressed={showPassword} aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"} title={showPassword ? "Ocultar senha" : "Mostrar senha"}>{showPassword ? <EyeOff size={19} /> : <Eye size={19} />}</button></div><button className="primary-action" onClick={() => { if (!controller.authorizeTeacher(password)) setError("Senha não reconhecida."); }}>Entrar <ChevronRight size={21} /></button>{error && <small className="login-error">{error}</small>}<button className="teacher-entry" onClick={() => controller.returnToMenu()}>Voltar ao menu</button></section></main>;
  const openTab = (next: TeacherTab) => { setTab(next); if (next !== "answers" && next !== "drawings") setSelectedAnswer(null); };
  return <main className="teacher-page"><Header state={state} controller={controller} back /><section className="teacher-layout"><aside className="teacher-sidebar"><p className="eyebrow">Painel do professor</p><h2>Acompanhamento<br />da turma</h2><button className={tab === "overview" ? "active" : ""} onClick={() => openTab("overview")}><BookOpen size={18} /> Visão geral</button><button className={tab === "profiles" ? "active" : ""} onClick={() => openTab("profiles")}><PawPrint size={18} /> Perfis de alunos</button><button className={tab === "answers" ? "active" : ""} onClick={() => openTab("answers")}><CircleHelp size={18} /> Respostas</button><button className={tab === "drawings" ? "active" : ""} onClick={() => openTab("drawings")}><Paintbrush size={18} /> Desenhos</button><button className="back-sidebar" onClick={() => controller.goToMap()}><ArrowLeft size={18} /> Voltar ao mapa</button></aside><section className="teacher-content"><div className="teacher-welcome"><div><p className="eyebrow">Aluno em foco</p><h1>{profile?.name || "Nenhum aluno cadastrado"}</h1><p>Todos os registros ficam neste navegador e podem ser analisados resposta por resposta.</p></div><div className="student-badge"><Mascot label="Parceira Lumi" /><span>{profile?.partner || "Parceiro"}</span></div></div><section className="local-students-card paper-panel"><div className="table-head"><div><p className="eyebrow"><PawPrint size={15} /> Turma conectada</p><h2>Alunos neste servidor</h2></div><span>{remoteStudents.length} aluno(s)</span></div>{remoteStudents.length ? <div className="local-students-grid">{remoteStudents.map((student) => { const completed = Object.keys(student.completions ?? {}).length; return <article key={student.id}><div className="student-mini-avatar"><PawPrint size={18} /></div><div><strong>{student.profile.name}</strong><small>Mundo {student.profile.currentWorld + 1} · {completed} fase(s) · {student.answers.length} resposta(s)</small><small>Pet: {student.profile.petName}</small></div></article>; })}</div> : <p className="empty-note">Os alunos aparecerão aqui assim que abrirem a aventura neste endereço.</p>}<small className="sync-note">Atualização automática a cada 4 segundos.</small></section><section className="teacher-profile-card paper-panel"><div className="teacher-profile-heading"><div className="teacher-avatar"><Lock size={22} /></div><div><p className="eyebrow">Perfil do professor</p><h2>{state.teacherName}</h2><small>Área protegida deste dispositivo</small></div></div><div className="teacher-profile-edit"><input value={teacherName} onChange={(event) => setTeacherName(event.target.value)} placeholder="Nome do professor" maxLength={32} /><button className="soft-action" onClick={() => controller.updateTeacherProfile(teacherName)}>Salvar perfil <Check size={16} /></button></div><button className="teacher-entry teacher-exit-button" onClick={() => controller.exitTeacher()}>Sair da área do professor</button></section><section className="teacher-password-card paper-panel"><div><p className="eyebrow"><Lock size={15} /> Segurança</p><h2>Alterar senha de acesso</h2><p>Use a senha atual para criar uma nova senha para a área do professor.</p></div><div className="teacher-password-fields"><div className="password-input-wrap"><input type={showCurrentPassword ? "text" : "password"} value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} placeholder="Senha atual" autoComplete="current-password" /><button type="button" className="password-eye" onClick={() => setShowCurrentPassword((visible) => !visible)} aria-label={showCurrentPassword ? "Ocultar senha atual" : "Mostrar senha atual"}>{showCurrentPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div><div className="password-input-wrap"><input type={showNewPassword ? "text" : "password"} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder="Nova senha (mínimo 6 caracteres)" autoComplete="new-password" /><button type="button" className="password-eye" onClick={() => setShowNewPassword((visible) => !visible)} aria-label={showNewPassword ? "Ocultar nova senha" : "Mostrar nova senha"}>{showNewPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div><div className="password-input-wrap"><input type={showConfirmPassword ? "text" : "password"} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Confirmar nova senha" autoComplete="new-password" /><button type="button" className="password-eye" onClick={() => setShowConfirmPassword((visible) => !visible)} aria-label={showConfirmPassword ? "Ocultar confirmação" : "Mostrar confirmação"}>{showConfirmPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></div><button className="primary-action compact" onClick={() => { const message = controller.changeTeacherPassword(currentPassword, newPassword, confirmPassword); setPasswordMessage(message ?? "Senha alterada com sucesso."); if (!message) { setCurrentPassword(""); setNewPassword(""); setConfirmPassword(""); } }}>Salvar nova senha <Check size={18} /></button>{passwordMessage && <small className={passwordMessage.includes("sucesso") ? "password-success" : "login-error"}>{passwordMessage}</small>}</section>{profile ? <TeacherDashboard state={state} controller={controller} tab={tab} onTabChange={openTab} selectedAnswer={selectedAnswer} onSelectAnswer={setSelectedAnswer} /> : <div className="empty-teacher paper-panel">Ainda não há aluno cadastrado. Crie uma aventura na tela inicial.</div>}</section></section></main>;
}
