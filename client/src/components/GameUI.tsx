...
type RemoteStudent = { id: string; profile: { name: string; currentWorld: number; recommendedWorld?: number; xp: number; petName: string }; completions: Record<string, unknown>; answers: unknown[]; updatedAt: string };
...
function RemoteTeacherDashboard({ student, tab, onUpdate }: { student: RemoteStudent; tab: TeacherTab; onUpdate: (student: RemoteStudent) => Promise<void> }) {
  ...
  if (tab === "profiles" || tab === "overview") return <><section className="student-profile-card paper-panel"><p className="eyebrow">Perfil completo do aluno</p><h2>{student.profile.name}</h2><p>Mundo atual: <strong>{student.profile.currentWorld + 1}</strong> · {student.profile.xp} XP · Pet: <strong>{student.profile.petName}</strong></p><div className="remote-performance-grid"><span><b>{completed}/{PHASES_PER_WORLD}</b> fases no mundo</span><span><b>{accuracy}%</b> desempenho</span><span><b>{student.answers.length}</b> respostas</span></div>{canAdvance ? <button className="release-button" onClick={() => onUpdate({ ...student, profile: { ...student.profile, currentWorld: student.profile.currentWorld + 1, recommendedWorld: student.profile.currentWorld + 1 } })}>Liberar próximo mundo <ChevronRight size={19} /></button> : <small className="sync-note">O próximo mundo será liberado após concluir todas as fases com pelo menos 70% de acerto.</small>}</section><section className="teacher-table paper-panel"><div className="table-head"><div><p className="eyebrow">Desempenho detalhado</p><h2>Atividades concluídas</h2></div><span>{completed} de {PHASES_PER_WORLD}</span></div><div className="world-teacher-grid">{WORLDS.map((world) => { const done = Object.keys(student.completions ?? {}).filter((key) => key.startsWith(`${world.id}:`)).length; return <div className="remote-world-row" key={world.id}><strong>{world.icon} {world.shortName}</strong><small>{done}/{PHASES_PER_WORLD} fases</small><i style={{ width: `${Math.min(100, done / PHASES_PER_WORLD * 100)}%` }} /></div>; })}</div></section></>;
  return <section className="analysis-workspace"><div className="analysis-list paper-panel"><div className="table-head"><div><p className="eyebrow">{tab === "drawings" ? "Portfólio visual" : "Resposta por resposta"}</p><h2>{tab === "drawings" ? "Desenhos enviados" : "Atividades e respostas"}</h2><small className="soft-note">Clique em qualquer registro para analisar a tentativa completa.</small></div><span>{visible.length} registro(s)</span></div>{visible.length ? <div className="response-select-list">{visible.map(({ answer, index }) => <button key={`${answer.at}-${index}`} className={selectedEntry?.index === index ? "selected" : ""} onClick={() => setSelectedIndex(index)}><span className={answer.correct ? "status-correct" : "status-help"}>{answer.correct ? "ACERTO" : "COM DICA"}</span><strong>{WORLDS[answer.worldId]?.shortName || "Mundo"} · Fase {answer.phase + 1}</strong><small>{answer.question}</small>{answer.drawing && <em className="drawing-attached">DESENHO ANEXADO</em>}</button>)}</div> : <p className="empty-note">Ainda não há registros nesta categoria.</p>}</div>{selectedEntry && <AnswerInspector answer={selectedEntry.answer} />}</section>;
}
