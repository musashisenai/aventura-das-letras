...
import { ArrowLeft, BookOpen, Check, ChevronRight, CircleHelp, Coins, Download, Eye, EyeOff, FileText, Gift, Heart, Lock, Paintbrush, PawPrint, Play, RotateCcw, Sparkles, Star, Volume2, VolumeX, X } from "lucide-react";
...
type TeacherTab = "overview" | "profiles" | "answers" | "drawings" | "teacher-profile";
  ...
  return (
    <main className="welcome-page">
      ...
      <section className="welcome-copy paper-panel">
        ...
        <button className="primary-action" onClick={async () => { setNameError(""); const saved = state.profile ? await controller.updateProfileName(name) : await controller.beginProfile(name); if (!saved) setNameError("Este nome já está sendo usado por outro aluno. Escolha outro nome para continuar."); }}>{state.profile ? "Salvar alterações" : "Começar a expedição"} <ChevronRight size={23} /></button>
        {nameError && <small className="login-error name-error">{nameError}</small>}
...
function Teacher({ state, controller }: Props) {
  ...
  const deleteSelected = async (confirmation: string) => {
    if (!selected) return;
    const result = await controller.deleteRemoteStudent(selected.id, confirmation);
...
function RemoteTeacherDashboard({ student, tab, onUpdate, onDelete }: { student: RemoteStudent; tab: TeacherTab; onUpdate: (student: RemoteStudent) => Promise<void>; onDelete: (confirmation: string) => Promise<void> }) {
  ...
  const accuracy = worldAnswers.length ? Math.round((worldAnswers.filter(({ answer }) => answer.correct).length / worldAnswers.length) * 100) : 0;
  const approval = student.worldApprovals?.[String(student.profile.currentWorld)];
  const canAdvance = completed >= PHASES_PER_WORLD && accuracy >= 70 && student.profile.currentWorld < WORLDS.length - 1;
...
function TeacherDashboard({ state, controller, tab, onTabChange, selectedAnswer, onSelectAnswer }: Props & { tab: TeacherTab; onTabChange: (tab: TeacherTab) => void; selectedAnswer: number | null; onSelectAnswer: (index: number | null) => void }) {
  ...
  if (tab === "answers" || tab === "drawings") return <div className="analysis-workspace"><ResponseExplorer entries={visible} selectedIndex={selected?.index ?? null} onSelectAnswer={onSelectAnswer} tab={tab} />{selected && <AnswerInspector answer={selected.answer} />}</div>;
